...
public final class EventHandler {
    ...
    private static void doDropsForGroup(LivingDropsEvent event, LivingEntity entity, Level world, MinecraftServer server, Player player, IEntityGroup group) {
        LootTable lootTable = server.getLootData().getLootTable(group.getLootTable());
        LootParams.Builder paramsBuilder = (new LootParams.Builder((ServerLevel) entity.level()))
        ...
        lootTable.getRandomItems(paramsBuilder.create(LootContextParamSets.ENTITY), stack -> {
            event.getDrops().add(new ItemEntity(world, entity.getX(), entity.getY(), entity.getZ(), stack));
        });
    }
